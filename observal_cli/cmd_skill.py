...
def _sparse_clone_skill_dir(git_url: str, skill_path: str, git_ref: str, dest: Path) -> bool:
    ...
    try:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            # Initialise, add remote, enable sparse checkout
            subprocess.run(["git", "init"], cwd=tmp_path, check=True, capture_output=True)
            subprocess.run(["git", "remote", "add", "origin", git_url], cwd=tmp_path, check=True, capture_output=True)
            subprocess.run(
                ["git", "config", "core.sparseCheckout", "true"], cwd=tmp_path, check=True, capture_output=True
            )
            subprocess.run(
                ["git", "fetch", "--filter=blob:none", "--depth=1", "origin", git_ref],
                cwd=tmp_path,
                check=True,
                capture_output=True,
            )
            ...
            subprocess.run(["git", "checkout", f"origin/{git_ref}"], cwd=tmp_path, check=True, capture_output=True)
...
def skill_install(
    ...
    Primary path: git sparse-clone using git_url + skill_path + git_ref from
    the install snippet.  Writes the complete directory tree to
    .agents/skills/<name>/ and symlinks .<agent>/skills/<name>/ if the agent
    config dir exists in the current project.

    Fallback: if git is unavailable and skill_md_content is cached in the
    snippet, writes a single SKILL.md entry point.
    """
    resolved = config.resolve_alias(skill_id)
    ...
    skill_git_url = skill_info.get("git_url")
    skill_path = skill_info.get("skill_path", "/")
    skill_git_ref = skill_info.get("git_ref", "main")
    skill_name = _sanitize_name(skill_info.get("name", "skill"))

    if not no_write:
        cwd = Path.cwd()
        canonical_base = cwd / ".agents" / "skills"
        dest = canonical_base / skill_name

        # Safety: prevent path traversal via crafted skill name
        if not _is_path_safe(dest, canonical_base):
        ...
        if skill_git_url:
            with spinner(f"Cloning skill from {skill_git_url} …"):
                dest.mkdir(parents=True, exist_ok=True)
                wrote_full_dir = _sparse_clone_skill_dir(skill_git_url, skill_path, skill_git_ref, dest)

            if wrote_full_dir:
                rprint(f"[green]✓ Skill directory written:[/green] {dest}")
                # Symlink into IDE config dirs that exist in the project
                _symlink_for_ides(cwd, dest, skill_name)
            else:
                rprint("[yellow]⚠ git clone failed or git not available.[/yellow] Falling back to SKILL.md cache.")

        if not wrote_full_dir:
            # Fallback: write single SKILL.md from cache
            skill_file = snippet.get("skill_file")
            cached_md = skill_info.get("skill_md_content")
            if skill_file:
                ...
                fp.write_text(skill_file["content"], encoding="utf-8")
                rprint(f"[green]✓ Wrote skill file (cached):[/green] {fp}")
            elif cached_md:
                fallback = Path(".agents") / "skills" / skill_name / "SKILL.md"
                fallback.parent.mkdir(parents=True, exist_ok=True)
                fallback.write_text(cached_md, encoding="utf-8")
                rprint(f"[green]✓ Wrote skill file (cached):[/green] {fallback}")
            else:
                rprint("[yellow]⚠ No skill content available to write.[/yellow]")
    else:
        rprint(f"[dim]Skill dir target (not written):[/dim] .agents/skills/{skill_name}/")
