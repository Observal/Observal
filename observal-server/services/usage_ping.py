...
async def _session_metrics() -> tuple[dict[str, int], dict[str, int | float], dict[str, int]]:
    ...
    try:
        response = await _query(
            ...
            "count(DISTINCT user_id) FILTER (WHERE user_id != '' AND last_event_time >= now() - INTERVAL 7 DAY) AS active_users_7d, "
            "count(DISTINCT user_id) FILTER (WHERE user_id != '' AND last_event_time >= now() - INTERVAL 30 DAY) AS active_users_30d, "
            "count(DISTINCT agent_id) FILTER (WHERE agent_id != '' AND last_event_time >= now() - INTERVAL 7 DAY) AS active_agents_7d, "
            "count(DISTINCT agent_id) FILTER (WHERE agent_id != '' AND last_event_time >= now() - INTERVAL 30 DAY) AS active_agents_30d, "
            ...
            "sum(cache_write_tokens) FILTER (WHERE last_event_time >= now() - INTERVAL 30 DAY) AS cache_write_tokens_30d, "
            ...
            "count(*) FILTER (WHERE last_event_time >= now() - INTERVAL 30 DAY AND tool_call_count > 0) AS sessions_with_tools_30d, "
