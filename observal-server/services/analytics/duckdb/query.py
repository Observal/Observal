...
async def query_recent_events(minutes: int = 60) -> dict:
    """Get recent session activity counts from session tables."""
    minutes = int(minutes)
    try:
        r = await _client._query(
            "SELECT sum(tool_call_count) AS tools, count(*) AS sessions "
            "FROM session_stats_agg "
            "WHERE last_event_time > now() - to_minutes(CAST($minutes AS BIGINT))",
            {"minutes": str(minutes)},
        )
        r.raise_for_status()
...
async def query_existing_for_dedup(
    ...
    sql = (
        ...
        "AND line_offset >= CAST($min_off AS INTEGER) AND line_offset <= CAST($max_off AS INTEGER)"
    )
    params = {
        "pid": project_id,
        "uid": user_id,
        "harness": harness,
        "sid": session_id,
        "min_off": str(min_offset),
        "max_off": str(max_offset),
    }
    try:
        r = await _client._query(sql, params)
        r.raise_for_status()
