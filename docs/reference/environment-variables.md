...
| `DUCKDB_ANALYTICS_URL`      | `duckdb://localhost:8484/observal`                               | DuckDB analytics service endpoint. Supports `DUCKDB_ANALYTICS_URL_FILE`. |
| `DUCKDB_ANALYTICS_TOKEN`    | empty (service rejects requests until set)                       | Bearer token shared with the analytics service. Supports `DUCKDB_ANALYTICS_TOKEN_FILE`. |
| `DUCKDB_MEMORY_LIMIT` | `2GB` | DuckDB's internal, process-wide memory limit |
| `DUCKDB_CONTAINER_MEMORY_LIMIT` | `3GB` | Compose container memory limit; keep above `DUCKDB_MEMORY_LIMIT` for runtime overhead |
...
| `DATA_RETENTION_DAYS` | `90`                                                             | Analytics retention in days, enforced by the purge job. `0` disables. Minimum non-zero: `7` |
