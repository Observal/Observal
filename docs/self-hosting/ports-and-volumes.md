...
| DuckDB | 8123 | `DUCKDB_HOST_PORT` | HTTP |
...
The worker has no exposed port; it talks to Redis and DuckDB internally only.
...
| `duckdbdata` | `/data` | DuckDB session, audit, and security data | High: all telemetry lost; accounts and registry survive |
...
* **Health checks** on Postgres, DuckDB, Redis, API.
...
| API / worker | DuckDB | `duckdb://observal-duckdb:8484/observal` |
