...
def test_schema_validation_score_too_low():
    """score_retention_days must be >= 30."""
    with pytest.raises(ValueError, match="score_retention_days must be >= 30"):
        RetentionConfigUpdate(retention_enabled=True, data_retention_days=14, score_retention_days=10)
