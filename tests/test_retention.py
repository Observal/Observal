...
async def test_delete_batch_uses_parameterized_query():
    """DELETE batch uses bound $pid and $cutoff placeholders."""
    with patch("services.analytics.duckdb._execute", new_callable=AsyncMock) as mock_execute:
        ...
        assert "$pid" in sql
        assert "CAST($cutoff AS TIMESTAMP)" in sql
        assert "pid" in params
        assert "cutoff" in params
        assert params["pid"] == "test-project-id"
        assert params["cutoff"] == "2026-04-01 00:00:00.000"
...
async def test_count_based_purge_uses_daily_aggregation():
    ...
    with (
        patch("services.analytics.duckdb._query", new_callable=AsyncMock) as mock_query,
        # Deletes go through the execute primitive; keep them off the network.
        patch("services.analytics.duckdb._execute", new_callable=AsyncMock),
    ):
...
async def test_has_data_check():
    ...
    with patch("services.analytics.duckdb._query", new_callable=AsyncMock) as mock_query:
